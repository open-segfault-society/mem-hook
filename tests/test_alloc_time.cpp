...
size_t static const NUM_ALLOCS{1000};
...
int main() {
    ...
    std::transform(std::begin(alloc_sizes), std::end(alloc_sizes),
                   std::back_inserter(results), [](size_t size) {
                       return run_test(size, NUM_ALLOCS) / NUM_ALLOCS;
                   });
